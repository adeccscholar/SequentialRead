...
void TProcesses::Read(std::string const& strFile) {
	...
	std::copy(file_data.begin(), file_data.end(), back_inserter(lines));
	theAddresses.resize(lines.size());

    #if !defined __BORLANDC__
	std::for_each(std::execution::par, lines.cbegin(), lines.cend(), [this](auto const& val) {
    #else
	std::for_each(lines.cbegin(), lines.cend(), [this](auto const& val) {
    #endif
		size_t iPos = 0, iEnd;
		int iCount = 0;
		do {
			iEnd = val.view.find(";", iPos);
			funcs[iCount++](this->theAddresses[val.index], val.view.substr(iPos, iEnd - iPos));
			if (iEnd != std::string_view::npos) iPos = iEnd + 1;
		} while (iEnd != std::string_view::npos);
		});
}

void TProcesses::Calculate(TGeoLocation const& from) {
    #if defined __BORLANDC__
	static double r = 6371000.785;        ///< mean radius of the earth
	static const double pi = std::acos(-1);     // 3.14159265359  since C++20 use std::numbers::pi
	static const double w1 = pi / 180.0;
	static const double w2 = 180.0 / pi;
    #else
	static constinit double r = 6371000.785;        ///< mean radius of the earth
	static constinit const double w1 = std::numbers::pi / 180.0;
	static constinit const double w2 = 180.0 / std::numbers::pi;
    #endif

	const double phiB = from.Latitude() * w1; /// 180.0 * M_PI;
	const double lambdaB = from.Longitude() * w1; /// 180.0 * M_PI;
	const double sin_phiB = std::sin(phiB);
	const double cos_phiB = std::cos(phiB);

    #if !defined __BORLANDC__
	for_each(std::execution::par, theAddresses.begin(), theAddresses.end(), [&phiB, &lambdaB, &sin_phiB, &cos_phiB](auto& val) mutable {
    #else
	for_each(theAddresses.begin(), theAddresses.end(), [&phiB, &lambdaB, &sin_phiB, &cos_phiB](auto& val) mutable {
    #endif
		double phiA    = val.Latitude()  * w1; /// 180.0 * M_PI;
		double lambdaA = val.Longitude() * w1; /// 180.0 * M_PI;

		double zeta = std::acos(std::sin(phiA) * sin_phiB + std::cos(phiA) * cos_phiB * std::cos(lambdaB - lambdaA));
		double tmp = (sin_phiB - std::sin(phiA) * std::cos(zeta)) / (std::cos(phiA) * std::sin(zeta));
		double alpha = tmp < -1 ? 180.0 : tmp > 1 ? 0 : std::acos(tmp) * w2;
		val.thePosition.Distance(zeta * r);
		val.thePosition.Angle(lambdaA > lambdaB ? phiA > 0 ? 360.0 - alpha : 180.0 + alpha : phiA > 0 ? alpha : 180.0 - alpha);
		});
